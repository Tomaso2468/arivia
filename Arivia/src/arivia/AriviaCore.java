...
//		loadSquareSize(tree);
		...
		bot = new EBot(tree, new ComplexTextComparer() {
			...
			public String pow(BigDecimal n1, BigDecimal n2, WorldInterface wi) throws Exception {
				...
				if (!(n1.compareTo(new BigDecimal(dn1)) == 0))
					throw new Exception("Cannot convert n1 to double"); // Cannot convert n1 to double
				...
				try {
					...
					if (n2IntPart.intValueExact() < 0) {
						...
						intPow = BigDecimal.ONE.divide(n1.pow(-n2IntPart.intValueExact(), MathContext.UNLIMITED),
					...
					if (n2IntPart.intValue() < 0) {
						...
						intPow = BigDecimal.ONE.divide(n1.pow(-n2IntPart.intValue(), MathContext.UNLIMITED),
								MathContext.UNLIMITED);
					} else {
			...
			public void doTask(String d, WorldInterface wi, Context c) {
				...
				if (((AriviaWorldInterface) wi).allowMaths()) {
						...
						try {
							...
							if (sum.toPlainString().length() > (2000 - "The answer is ".length())) {
								...
								for (int i = 0; i < output.length(); i += 1000) {
									...
									String s = (i > 0 ? ".." : "") + output.substring(i, m)
											+ (m == output.length() - 1 ? "" : "...");
					...
					if (d.toLowerCase().contains("^")) {
						System.out.println("Text contains power");
						try {
							if (d.toLowerCase().startsWith("what is")) {
								d = d.toLowerCase().substring(7);
							}
					...
					if (d.toLowerCase().startsWith("define ")) {
						...
						try {
							URL url = new URL("https://mashape-community-urban-dictionary.p.rapidapi.com/define?term="
									+ d.toLowerCase().substring(d.toLowerCase().indexOf(' ') + 1).replace("&", "%26")
											.replace(" ", "%20"));
							HttpsURLConnection con = (HttpsURLConnection) url.openConnection();
							con.setRequestMethod("GET");

							con.addRequestProperty("x-rapidapi-host",
							...
							String s = array.getJSONObject((int) (Math.random() * array.length() - 1))
...
//				new FileDataSource("sarcasmN.txt"), 0.9, 1000));
	...
	public void acceptConnection(Connection cn) {
		WorldInterface wi = new AriviaWorldInterface() {
			@Override
